...
function ensureServiceNodeModulesLink(repoRoot: string, workspaceRoot: string) {
  ...
  if (!fs.existsSync(sourceNodeModules)) {
    return {
      ok: false,
      reason: `Source node_modules not found: ${sourceNodeModules}`,
      sourceNodeModules,
      targetNodeModules,
    };
  }
  ...
  try {
    ...
    return {
      ok: false,
      reason: error?.message || "Failed to create node_modules junction",
      sourceNodeModules,
      targetNodeModules,
    };
  }
...
export async function runLocalChecksForProposal(proposalId: string) {
  ...
  const nodeModulesLink = ensureServiceNodeModulesLink(repoRoot, workspaceRoot);

  if (!nodeModulesLink.ok) {
    return {
      statusCode: 500,
      error: `Unable to prepare workspace dependencies: ${nodeModulesLink.reason}`,
    };
  }
  ...
  const action = await createPrAction({
    ...
    payload: {
      ...
      workspace_dependencies: {
        service_b_node_modules_linked: true,
        source_node_modules: nodeModulesLink.sourceNodeModules,
        target_node_modules: nodeModulesLink.targetNodeModules,
      },
