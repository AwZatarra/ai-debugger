...
import { createGithubPrForProposal } from "./prProposalCreateGithubPr";
...
app.post("/pr-proposals/:proposalId/create-github-pr", async (req, res) => {
  try {
    const proposalId = req.params.proposalId;
    const created = await createGithubPrForProposal(proposalId);
    ...
    console.error("Create GitHub PR error:", error.message);
    res.status(500).json({
      ok: false,
      error: error.message,
    });
